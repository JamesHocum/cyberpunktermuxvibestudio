...
import { Copy, Check, FileDown, FileCode, Rocket } from "lucide-react";
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-studio-header border-b border-border/30">
        ...
        <div className="flex items-center gap-1">
          <Button
            variant="ghost"
            size="sm"
            className="h-5 px-1.5 text-[10px] font-terminal text-muted-foreground hover:text-foreground"
            onClick={handleCopy}
          >
          ...
          {onApply && (
            <Button
              variant="ghost"
              size="sm"
              className={`h-5 px-2 text-[10px] font-terminal gap-1 ${
                applied
                  ? 'text-green-400'
...
export const MessageContent: React.FC<MessageContentProps> = ({ content, onApplyCode }) => {
  ...
  return (
    <div>
      ...
      {/* Apply All button when multiple code blocks */}
      {codeBlocks.length > 1 && onApplyCode && (
        <div className="mt-3 flex justify-end">
          <Button
            variant="outline"
            size="sm"
            className={`text-xs font-terminal gap-1.5 cyber-border ${
              allApplied
          ...
          >
            {allApplied ? (
              ...
              <>
                ...
                Apply All ({codeBlocks.length} files)
              </>
            )}
          </Button>
        </div>
