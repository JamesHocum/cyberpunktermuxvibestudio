...
const BUILD_COMMANDS: BuildCommand[] = [
  ...
  },
  {
    ...
    description: 'Compile Vite frontend for Electron'
  },
  {
    label: 'Windows Installer',
    command: 'npx electron-builder --win nsis',
    description: 'Creates NSIS installer (.exe)'
  },
  {
    label: 'Windows Portable',
    command: 'npx electron-builder --win portable',
    description: 'Creates portable executable'
  },
  {
    label: 'macOS DMG',
    command: 'npx electron-builder --mac',
    description: 'Creates macOS disk image'
  },
  {
    label: 'Linux AppImage',
    command: 'npx electron-builder --linux',
    description: 'Creates Linux AppImage'
  },
];
    ...
    <ScrollArea className="h-full p-4">
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {BUILD_COMMANDS.map((cmd) => (
              <div
                key={cmd.command}
                className="p-2 rounded-lg cyber-border bg-studio-terminal group hover:bg-primary/5 transition-colors"
              >
                <div className="flex items-center justify-between">
                  ...
                  >
                    {copiedCommand === cmd.command ? (
                      <CheckCircle2 className="h-3 w-3 neon-green" />
                    ) : (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {OUTPUT_FILES.map((output, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between p-2 rounded-lg cyber-border bg-studio-terminal"
              >
          ...
          <div className="p-3 rounded-lg cyber-border bg-studio-terminal">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs">
                <code className="px-2 py-1 rounded bg-primary/10 font-terminal neon-green">
                  git tag v1.0.0
                </code>
              </div>
              <div className="flex items-center gap-2 text-xs">
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-lg cyber-border bg-studio-terminal">
            <p className="text-[10px] text-muted-foreground font-terminal mb-2">
  ...
  win: {
    target: ['nsis', 'portable']
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            className="flex-1 neon-green text-xs"
            onClick={() => window.open('https://www.electron.build/', '_blank')}
          >
